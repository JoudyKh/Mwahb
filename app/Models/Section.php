...
class Section extends Model
{
    ...
    {
        $firstTeacehr = $this->teachers()->first();
        return $firstTeacehr ?? null;
    }
    ...
    public function scopeWithSubSectionLessonTimes(Builder $query, $section_id = null)
    {
        $query
            ->leftJoin('lessons', function ($join) {
                $join->on('sections.id', '=', 'lessons.section_id')
            ...
            SEC_TO_TIME(COALESCE(SUM(TIME_TO_SEC(lessons.time)), 0)) as total_lessons_time,
    ...
    public static function eagerLoadRelations(Section &$section)
    {
        ...
        //todo fix relations array to have all keys , 500 found with undefined super key
    ...
    {
        return $section->join('lessons', 'lessons.section_id', '=', 'sections.id')

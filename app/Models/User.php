...
class User extends Authenticatable
{
    ...
    public function scopeWithTopStudentsQuery($builder, $orderBy = [], $studentExamOrder = 'ASC', $allAttempts = false)
    {
        $builder
            ->leftJoinSub(
            ...
            ")->whereNull('exams.deleted_at'),
                'RankedStudentExams',
                function ($join) use ($allAttempts) {
                    $join->on('RankedStudentExams.student_id', '=', 'users.id')
                        ->when(!$allAttempts, function ($query) {
                            $query->where('RankedStudentExams.row_num', '=', 1);
                        });
                }
            )
            ...
            SUM(
                CASE
                    WHEN
                        (
                            RankedStudentExams.total_degree IS NOT NULL
                            AND RankedStudentExams.start_date IS NOT NULL
                            AND ((RankedStudentExams.degree * 100) / RankedStudentExams.total_degree) < RankedStudentExams.exam_pass_percentage
                        )

...
class LessonService
{
    ...
    public function getAll(Request &$request, string|int $parentSectionId)
    {
        ...
        if ($studentId and !request()->is('*admin*')) {

            $certificate = CertificateRequest::where([
                'student_id' => $studentId,
                'course_id' => $parentSectionId,
            ])
                ->first();

            $extraData['certificate'] = $certificate ? CertificateRequestResource::make($certificate) : null;

            $extraData['final_exam_id'] = null;

            $extraData['can_solve_final_exam'] =
                $extraData['can_apply_for_certificate'] = false;

            $lastLessonIsOpen =
                $lastLessonIsSolved =
                $lastExamIsSolved = false;

            $final_exam = Exam::with([
                'questions',
                'studentExams' => function ($query) use ($studentId) {
                    $query->where('student_exams.student_id', $studentId)
                        ->orderByDesc('created_at');
                }
            ])->where([
                        'model_id' => $parentSectionId,
            ...
            $lastLesson = Lesson::with([
                'exam',
                'exam.studentExams' => function ($query) use ($studentId) {
                    $query->where('student_exams.student_id', $studentId)
                        ->orderByDesc('created_at');
                },
                'lessonStudents' => function ($query) use ($studentId) {
                    $query->where('lesson_student.student_id', $studentId);
                }
            ])
                ->where('lessons.section_id', $section?->id)
                ->orderByDesc('lessons.lesson_order')//the orders in reverse because i use first method
                ->orderByDesc('lessons.created_at')
                ->first();

            if ($lastLesson) {
                if ($lastLesson?->lessonStudents?->count() > 0) {
                    $lastLessonIsOpen = true;
                }
                if ($lastLessonIsOpen) {
                    if (Lesson::isPassedLesson($lastLesson)) {
                        $lastLessonIsSolved = true;
                    }
                }

                if ($lastLessonIsSolved and $final_exam) {
                    $studentExam = $final_exam->studentExams->where('student_id', $studentId)->first();

                    $lastExamIsSolved = StudentExam::isPassedTheExam($studentExam);
                }

                $extraData['final_exam'] = $final_exam ? ExamResource::make($final_exam) : null;
                $extraData['final_exam_id'] = $final_exam?->id;
                $extraData['can_solve_final_exam'] = $lastLessonIsSolved;
                $extraData['can_apply_for_certificate'] =
                    (
                        $lastLessonIsOpen
                        and $lastLessonIsSolved
                        and (!$final_exam or ($final_exam?->questions?->count() === 0) or $lastExamIsSolved)
                    );
            }
        }
    ...
    public function get(Section &$parentSection, Lesson &$lesson, GetLessonRequest &$request)
    {
        ...
        if ($user != null and $user->hasRole(Constants::STUDENT_ROLE)) {
            ...
            if ($lessonStudent) {
                ...
                if ($lesson->section_id) {
                    ...
                    if (
                        $nextLessonId == -1 or ($nextLessonId and LessonStudent::where([
        ...
        // $videoId = explode('/', $lesson->video_url);
        // $videoId = end($videoId);

        if($lesson->video_url){
            $videoId = extractYouTubeVideoId($lesson->video_url);
            $lesson->video_streams = $videoId ? getVideoStreams($videoId) : [];
        }

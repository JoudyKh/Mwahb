...
class InfoService
{
    ...
    {
        $local = App::getLocale();
        $isAdmin = Auth::user()?->hasRole([Constants::ADMIN_ROLE, Constants::SUPER_ADMIN_ROLE]);
        $cacheKey = ($isAdmin ? 'admin' : 'app') . 'info.' . ($isAdmin ? 'all' : (request()->header('locale') ?? App::getLocale()));

...
class ExamService
{
    ...
    public function getAll(GetAllExamsRequest &$request, $authStudentId = null)
    {
        ...
        if ($request->has('type')) {
            $exams->where('type', strtoupper($request->get('type')));
            if (strtoupper($request->get('type')) === 'GENERATED' && !$currUser->hasRole(Constants::STUDENT_ROLE)) {
                $exams->whereNull('student_id');
            }
        }
    ...
    public function getExamStudents(Exam $exam)
    {
        ...
        $transformedResults = $paginatedResults->getCollection()->map(function ($result) {
            $percentage = ($result->degree / $result->total_degree) * 100;
            return [
                'student_id' => $result->student_id,
                'student_name' => $result->student->first_name . ' ' . $result->student->last_name,
                'marks_as_percentage' => round($percentage, 2),
                'student_marks' => $result->exam_degree * ($percentage / 100),
                'total_marks' => $result->exam_degree,
            ];
        });
    ...
    public function getExam(Exam &$exam)
    {
        ...
        if (!request()->is('*admin*')) {
            ...
            $exam->loadMissing([
                'questions' => function ($query) use (&$exam) {
                    $query
                        ->orderByDesc('exam_question.created_at')
                        ->with(['options'])
                    ;
                }
            ]);

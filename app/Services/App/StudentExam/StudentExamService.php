...
class StudentExamService
{
    ...
    public static function openFirstExam(string|int $sectionId, string|int|null $studentId = null, $depthSectionLevel = 0)
    {
        ...
        //from the curr section , 0 been no join
    ...
    public function createTransaction(CreateStudentExamRequest &$request, Exam $exam)
    {
        return DB::transaction(function () use (&$request, $exam) {
            ...
            if (!$studentExam and $exam->model_type == Section::class) {

                $lastLesson = Lesson::with([
                    'exam.questions',
                    'lessonStudents' => function ($query) use ($user) {
                        $query->where('lesson_student.student_id', $user->id);
                    }
                ])
                    ->where('lessons.section_id', $exam->model_id)
                    ->orderByDesc('lesson_order')
                    ->first();

                if ($lastLesson) {
                    if (!$lastLesson->lessonStudents->contains('student_id', $user->id)) {
                        return error('you didnt open the last lesson in the course yet', [
                            'lesson_id' => $lastLesson->id
                        ], 403);
                    }

                    if (!Lesson::isPassedLesson($lastLesson)) {
                        return error('you didnt solve the last lesson in the course yet', [
                            'lesson_id' => $lastLesson->id
                        ], 403);
                    } else {
                        //the last lesson is open and solve but the creation or student exam failed somehow
                        //maybe the admin created the final exam after
                        $studentExam = $this->createNewStudentExam($exam, $user);
                        return StudentExamResource::make($studentExam);
                    }

                } else {
                    //section has only final exam
                    $studentExam = $this->createNewStudentExam($exam, $user);
                    return StudentExamResource::make($studentExam);
                }
            }
            ...
            if (!$studentExam and $exam->model_type == Lesson::class) {
                ...
                if (!$lessonStudent) {
                    ...
                    if (Section::getFirstLesson($section)) {
                        ...
                        return error('you didnt open the lesson with id : ' . $exam->model_id . ' yet', [], 403);
                    }
                } else {
                    $studentExam = $this->createNewStudentExam($exam, $user);
                    return StudentExamResource::make($studentExam);
    ...
    public function solve(Exam &$exam, StoreStudentExamRequest &$request)
    {
        ...
        $allowAfterSuccess = false;
        $showAnswersAfterFail = false;
